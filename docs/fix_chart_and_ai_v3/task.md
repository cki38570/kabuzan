...
- [x] ブラウザツールを使用して本番環境 (https://kabuzan-ai.streamlit.app/) の表示状態を確認する [x]
- [x] コンソールエラーや描画失敗の兆候を特定する [x]
- [x] `modules/charts.py` のボリンジャーバンド描画ロジックを再検討（カラム名の一致確認） [x]
- [x] `lightweight-charts` の最新仕様（特に `create_line` の `name` や `color` の渡し方）を再確認する [x]
